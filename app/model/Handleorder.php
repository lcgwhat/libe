...
class Handleorder extends Model
{
    public function __construct($orderid, $money, $cardnum = '', $cardvalue = '', $cardstate = '')
    {
        ...
        $this->money = number_format($money, 2, '.', '');//订单金额
    ...
    {
        $orders = $this->getOrder();//查询订单号
        if (!$orders || $orders['is_state'] > 0) {
          return false;
         }
        $rate = $this->getPrice($orders['userid'], $orders['channelid']); //汇率

        $data = array('realmoney' => $this->money, 'uprice' => $rate['uprice'], 'gprice' => $rate['gprice'], 'wprice' => $rate['wprice'], 'is_state' => 1, 'lastime' => time());

        $this->model()->from('orders')->updateSet($data)->where(array('fields' => 'orderid=?', 'values' => array($this->orderid)))->update();//订单更新
    ...
    private function getOrder()//查询订单号
    {
