...
# 2016-07 v2.0
# 程序作者:yang0 62561bd4206039f23425584eb203b4116e29d570
...
class apisubmitcard extends api{
    public function index(){
        ...
        {
            echo $this->ret->put('200',true);exit;
        }
            ...
            {
                echo $this->ret->put('304',true);exit;
            }
            if(!$acw=$this->model()->select()->from('acw')->where(array('fields'=>'code=?','values'=>array($paytype)))->fetchRow())
            {
                echo $this->ret->put('500',true);exit;
            }
            if($acw['length'])
            {
                $cardLength=json_decode($acw['length'],true);if(strlen($cardnum)!=$cardLength[0]){echo $this->ret->put('305',true);exit;
            }
            if(strlen($cardpwd)!=$cardLength[1])
            {
                echo $this->ret->put('306',true);exit;}}$acc=$this->model()->select('a.id,a.acpcode,a.gateway,a.is_state,b.is_state as is_state_acc,b.channelid')->from('acc a')->left('userprice b')->on('b.channelid=a.id')->join()->where(array('fields'=>'b.userid=? and a.acwid=?','values'=>array($customerid,$acw['id'])))->fetchRow();
            if(!$acc)
            {
                echo $this->ret->put('103',true);exit;
            }
            if($acc['is_state']=='1'){echo $this->ret->put('100',true);exit;}if($acc['is_state_acc']=='1')
            {
                echo $this->ret->put('102',true);exit;
            }
            $channelid=$acc['channelid'];$acpcode=$acc['acpcode'];$gateway=$acc['gateway'];
            ...
            {
                echo $this->ret->put('209',true);exit;
            }
            ...
            $http->toUrl();
            $content=$http->getResContent();$code=$http->getResCode();
            ...
            if($content=='ok'){
                echo $this->ret->put('302',true);exit;
            }
